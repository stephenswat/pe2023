...
/* 
 * for nz=1 - dense matrix; for nz>1 - sparse matrix with nz elements non-0; for nz=0: error
 * very naive generation - can be improved and made more diverse, but this is not part of the assignment ,,, 
 */
void generate_mat(int m, int n, float *A, int nz) {
  int i;
  int freq = m*n/nz; // to make nz elements, every "freq" element is non-zero; 
